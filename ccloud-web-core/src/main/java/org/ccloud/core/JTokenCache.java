...
public class JTokenCache implements ITokenCache {
	...
	public void put(Token token) {
		JCacheKit.put("token", token.getId(), token);
	}

	@Override
	public void remove(Token token) {
		JCacheKit.remove("token", token.getId());
	}

	@Override
	public boolean contains(Token token) {
		@SuppressWarnings("unchecked")
		List<String> keys = (List<String>)JCacheKit.getKeys("token");
	...
	public List<Token> getAll() {
		...
		try {
			...
			if (null != keys && keys.size() > 0) {
				for (String key : keys) {
					ret.add((Token) JCacheKit.get("token", key));
				}
			}
		} catch (Exception e) {
