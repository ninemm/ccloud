...
CREATE TABLE `{table_prefix}res` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `pid` bigint(20) unsigned DEFAULT NULL COMMENT '',
  ...
  `icon_cls` varchar(255) DEFAULT 'am-icon-file' COMMENT 'icon图标',
  ...
  `key` varchar(255) DEFAULT NULL COMMENT '资源标识',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='系统资源信息';
...
CREATE TABLE `{table_prefix}role` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `name` varchar(111) NOT NULL COMMENT '角色名称',
  `des` varchar(255) DEFAULT NULL COMMENT '角色描述',
  `seq` int(11) DEFAULT '1' COMMENT '排序',
...
CREATE TABLE `{table_prefix}role_res` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `role_id` bigint(20) unsigned NOT NULL COMMENT '角色ID',
  `res_id` bigint(20) unsigned NOT NULL COMMENT '资源ID',
  `created` datetime NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_sys_ROLE_RES_RES_ID` (`res_id`),
  KEY `FK_sys_ROLE_RES_ROLE_ID` (`role_id`),
  CONSTRAINT `sys_role_res_ibfk_1` FOREIGN KEY (`res_id`) REFERENCES `{table_prefix}res` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `sys_role_res_ibfk_2` FOREIGN KEY (`role_id`) REFERENCES `{table_prefix}role` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
