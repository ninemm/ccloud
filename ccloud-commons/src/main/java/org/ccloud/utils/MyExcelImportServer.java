...
public class MyExcelImportServer extends ImportBaseService {

    private final static Logger LOGGER = LoggerFactory.getLogger(MyExcelImportServer.class);
    ...
    private <T> List<T> importExcel(Collection<T> result, Sheet sheet, Class<?> pojoClass,
        ...
        if(params.getStartRows() >= totalRows)
        	return collection;
        //跳过无效行
    ...
    public ExcelImportResult importExcelByIs(InputStream inputstream, Class<?> pojoClass,
        ...
        long createBookStartTime = System.currentTimeMillis();
        ...
        long createBookEndTime = System.currentTimeMillis();
        System.out.println("创建book耗时：" + (createBookEndTime - createBookStartTime));
        ...
        for (int i = params.getStartSheetIndex(); i < params.getStartSheetIndex()
            ...
            if (isXSSFWorkbook) {
                pictures = PoiPublicUtil.getSheetPictrues07((XSSFSheet) sheet,
                        (XSSFWorkbook) book);
            } else {
                pictures = PoiPublicUtil.getSheetPictrues03((HSSFSheet) sheet,
                        (HSSFWorkbook) book);
            }
            if (LOGGER.isDebugEnabled()) {
            ...
            int totalRows = sheet.getPhysicalNumberOfRows();
            int readRows = totalRows % threadCount > 0 ? (totalRows / threadCount + 1) : totalRows / threadCount;
            params.setStartRows(threadIndex * readRows);
            params.setReadRows(readRows);
            long readExcelStartTime = System.currentTimeMillis();
            result.addAll(importExcel(result, sheet, pojoClass, params, pictures));
            long readExcelEndTime = System.currentTimeMillis();
            System.out.println("read excel耗时：" + (readExcelEndTime - readExcelStartTime));
            if (LOGGER.isDebugEnabled()) {
