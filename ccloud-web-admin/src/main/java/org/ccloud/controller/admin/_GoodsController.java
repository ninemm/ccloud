...
public class _GoodsController extends JBaseCRUDController<Goods> { 
	...
	private boolean saveProductInfo(Map<String, String[]> map, Goods goods, boolean update) {
		...
		if (loop > 0) {
			for (int i = 0; i >= 0; i++) {
				...
				for (ProductGoodsSpecificationValue ccProductGoodsSpecificationValue : cpsvList) {
					ccProductGoodsSpecificationValue.delete();
				}
				
				List<GoodsGoodsSpecification> ggsList = GoodsGoodsSpecificationQuery.me().findByGoodsId(goods.getId());
				for (GoodsGoodsSpecification goodsGoodsSpecification : ggsList) {
				...
				for (int j = 0;j < ids.length;j++) {
					...
					try {
						ProductGoodsSpecificationValue pgsValue = new ProductGoodsSpecificationValue();
		...
		for (GoodsAttribute goodsAttribute : attributes) {
			...
			if (attribute != null) {
				GoodsGoodsAttributeMapStore attributeMapStore = new GoodsGoodsAttributeMapStore();
