...
public class _ContentController extends JBaseCRUDController<Content> {
	...
	public void index() {
		...
		for (String stringid : tidStrings) {
			if (StringUtils.isNotBlank(stringid)) {
				tidList.add(stringid);
			}
		}
		tids = tidList.toArray(new String[] {});
	...
	private void processSelected(String[] tids, List<Taxonomy> taxonomys) {
	...
	public void edit() {
		...
		setAttr("urlPreffix", ContentRouter.getContentRouterPrefix(module.getName()));
		...
		for (TplTaxonomyType type : types) {
			if (type.isInputType()) {
				...
				for (String slug : slugs) {
					String id = getTaxonomyIdFromListBySlug(slug, taxonomyList);
					if (id == null) {
				...
				String[] ids = getParaValues("_" + type.getName());
	...
	private String getTaxonomyIdFromListBySlug(String slug, List<Taxonomy> list) {
	...
	public void save() {
		...
		boolean saved = Db.tx(new IAtom() {
			...
			public boolean run() throws SQLException {
				...
				List<String> ids = getOrCreateTaxonomyIds(content.getModule());
				if (ids == null || ids.size() == 0) {
					MappingQuery.me().deleteByContentId(content.getId());
				} else {
					if (!MappingQuery.me().doBatchUpdate(content.getId(), ids.toArray(new String[0]))) {
						return false;
					}
				}
				...
				MessageKit.sendMessage(Actions.CONTENT_COUNT_UPDATE, ids.toArray(new String[] {}));

				if (oldContent != null && oldContent.getTaxonomys() != null) {
					List<Taxonomy> taxonomys = oldContent.getTaxonomys();
					String[] taxonomyIds = new String[taxonomys.size()];
	...
	@Before({WechatUserInterceptor.class, WechatApiConfigInterceptor.class})
	public void syncContentToWx() {
		
		Object _state = getPara("state");
		Object _wechatSession = this.getSessionAttr(Consts.SESSION_WECHAT_USER);
		ApiResult _result = _wechatSession != null ? ApiResult.create(_wechatSession.toString()) : null;
		String[] ids = _state != null ? _state.toString().split("ZZZ") : null;
		...
		String openId = _result.getStr("openid");
		...
		for (String contentId : ids) {
			...
			articles.setTitle(content.getTitle());
			articles.setPicurl(content.getThumbnail());
			articles.setUrl(content.getUrl());
			articles.setDescription(content.getSummary());
			list.add(articles);
		}

		ApiResult result = CustomServiceApi.sendNews(openId, list);
		if(result.isSucceed())
			render(result.getJson());
		else
			renderNull();
