...
public class _SystemsController extends JBaseCRUDController<Systems> { 
	...
	public void refresh() {
		...
		if(!hasKey || (hasKey && cacheNames.length > 1))
			return;
			String[] currentCacheNames = null;
			try {
				SAXReader saxReader = new SAXReader();
				Document document = saxReader.read(new File(PathKit.getWebRootPath() + "ehcache.xml")); // 读取XML文件,获得document对象
				Element rootElement = document.getRootElement();
				List<Element> cache = rootElement.elements("cache");
				currentCacheNames = new String[cache.size()];
				for (int i = 0; i < cache.size(); i++) {
					currentCacheNames[i] = cache.get(i).attribute("name").getValue();
				}
...
//		System.out.println("cache list============\n" + ToStringBuilder.reflectionToString(currentCacheNames, ToStringStyle.MULTI_LINE_STYLE));
		if(ArrayUtils.contains(currentCacheNames, cacheNames[0]))
			JCacheKit.removeAll(cacheNames[0]);
//		List list = JCacheKit.getKeys(cacheNames[0]);
