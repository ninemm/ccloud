...
import org.ccloud.business.CustomerImportBiz;
...
public class _SellerCustomerController extends JBaseCRUDController<SellerCustomer> {
	...
	public void uploading() {
		...
		CustomerImportResult importResult = CustomerImportBiz.me().handleCustomerImport(file, user, sellerId, dataArea, dept_dataArea, userIds, dealerDataArea);
		
		System.out.println("=========导入客户耗时：" + (System.currentTimeMillis() - startTime));
		StringBuilder importResultBuilder = new StringBuilder();
		importResultBuilder.append("成功导入客户");
		importResultBuilder.append(importResult.getSuccessNum() != null ? importResult.getSuccessNum() : 0);
		importResultBuilder.append("个,已存在客户");
		importResultBuilder.append(importResult.getExistNum() != null ? importResult.getExistNum() : 0);
		importResultBuilder.append("个");
		if(importResult.getErrorRowIndexes().size() > 0) {
			importResultBuilder.append(",客户类型不存在行数:" + importResult.getErrorRowIndexes().size());
//			List<String> errorRowIndexes = importResult.getErrorRowIndexes();
//			importResultBuilder.append(StrKit.join(errorRowIndexes.toArray(new String[errorRowIndexes.size()]), ","));
		}
		renderAjaxResultForSuccess(importResultBuilder.toString());
	}
	...
	public void batchSetUser() {
		...
		for (String sellerCustomerId : sellerCustomerIds) {
			...
			for (String userId : userIds)
				addUserJoinCustomers.addAll(getInsertUserJoinCustomers(sellerCustomerId, userId));
		}
		if(addUserJoinCustomers.size() > 0)
			Db.batchSave(addUserJoinCustomers, addUserJoinCustomers.size());
