...
public class ActivityApplyQuery extends JBaseQuery { 
	...
	public List<Record> findBySellerCustomerId(String sellerCustomerId) {
		return Db.find(" select ap.id, a.title from cc_activity_apply ap join cc_activity a on ap.activity_id = a.id where ap.seller_customer_id = ? and ap.status in ("+Consts.ACTIVITY_APPLY_STATUS_PASS+","+Consts.ACTIVITY_APPLY_STATUS_VERIFICATION+") order by ap.status, ap.create_date desc", sellerCustomerId);
	}
