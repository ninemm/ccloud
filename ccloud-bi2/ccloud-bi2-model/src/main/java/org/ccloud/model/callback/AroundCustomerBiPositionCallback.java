...
public class AroundCustomerBiPositionCallback implements ICallback {
	...
	public Object call(Connection conn) {
		...
		try {
			proc = conn.prepareCall("{ call around_customer_bi_position(?, ?, ?, ?, ?, ?, ?) }");
			...
			while (resultSet.next()) {
				...
				map.put("baiduDist", resultSet.getDouble("baiduDist"));
				result.add(map);
			}
