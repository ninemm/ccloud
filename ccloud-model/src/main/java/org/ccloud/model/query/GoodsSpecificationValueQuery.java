...
public class GoodsSpecificationValueQuery extends JBaseQuery { 
	...
	public List<GoodsSpecificationValueInfo> findByProductId(String productId) {
 		StringBuilder fromBuilder = new StringBuilder("SELECT c.id,c.name,c.goods_specification_id,c.image_path,c.order_list,c.create_date,c.modify_date ");
		...
		List<Record> list = Db.find(fromBuilder.toString(), productId);
		List<GoodsSpecificationValueInfo> sInfoList = new ArrayList<>();
		for (Record record : list) {
		GoodsSpecificationValueInfo gInfo = new GoodsSpecificationValueInfo();
		gInfo.setId(record.getStr("id"));	
		gInfo.setName(record.getStr("name"));
		gInfo.setImagePath(record.getStr("image_path"));
		gInfo.setCreateDate(record.getDate("create_date"));
		gInfo.setModifyDate(record.getDate("modify_date"));
		gInfo.setGoodsSpecificationId(record.getStr("goods_specification_id"));
		gInfo.setOrderList(record.getInt("order_list"));
		sInfoList.add(gInfo);
		}
		return sInfoList;
	}	
