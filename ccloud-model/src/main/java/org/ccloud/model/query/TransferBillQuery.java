...
public class TransferBillQuery extends JBaseQuery {
	...
	public Record findByTransferBillSn(String sn) {

		String sql = "SELECT( SELECT w.`name` FROM cc_warehouse w WHERE w.id = tb.from_warehouse_id) from_warehouse_name ,( SELECT w.`name` FROM cc_warehouse w WHERE w.id = tb.to_warehouse_id) to_warehouse_name ,";
		sql = sql + "( SELECT s.seller_name FROM cc_seller s WHERE s.id = tb.seller_id) seller_name ,( SELECT u.realname FROM `user` u WHERE u.id = tb.input_user_id) realname , tb.* FROM cc_transfer_bill tb";
		sql = sql + " WHERE tb.transfer_bill_sn = '" + sn + "'";
