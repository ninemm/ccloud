...
//		needWhere = appendIfNotEmpty(fromBuilder, "o.customer_type_id", customerTypeId, params, needWhere);
	...
	public Record findRecordById(final String id) {
		StringBuilder fromBuilder = new StringBuilder(" select so.`status`,so.modify_date,so.order_sn, so.create_date AS createDate, u.realname AS salesName, sos.outstock_sn, sos.create_date AS passDate, uu.realname AS outName, sos.biz_date AS outDate ");
		fromBuilder.append(" from `cc_sales_order` so ");
