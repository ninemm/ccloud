...
public class RoleQuery extends JBaseQuery {
	...
	public List<Role> findByUserId(String userId) {
		List<Role> roleList = JCacheKit.get(Role.CACHE_NAME, RedisConsts.REDIS_KEY_USER_ROLE_LIST.concat(userId));
		if (roleList == null) {
			...
			roleList = DAO.find(sqlBuilder.toString(), userId);

			JCacheKit.put(Role.CACHE_NAME, RedisConsts.REDIS_KEY_USER_ROLE_LIST.concat(userId), roleList);
		}
		return roleList;
	}
	...
	public List<Role> _findByDeptId(String deptId) {
		String sql = "SELECT * from `role` where dept_id = '"+deptId+"' and role_code >'001' and role_code<'022'";
		return DAO.find(sql);
	}

	//获取所有的角色
	public List<Record> findByRoleCheck( String dataArea) {
		List<Record> records = JCacheKit.get(Role.CACHE_NAME, RedisConsts.REDIS_KEY_AREA_ROLES_RECORD.concat(dataArea));
		if(records != null && records.size() > 0)
			return records;
		StringBuilder stringBuilder = new StringBuilder("SELECT r.id , r.role_name FROM role r WHERE r.data_area LIKE ?");
		records = Db.find(stringBuilder.toString(), dataArea);
		if(records != null && records.size() > 0)
			JCacheKit.put(Role.CACHE_NAME, RedisConsts.REDIS_KEY_AREA_ROLES_RECORD.concat(dataArea), records);
		return records;
	}
	//获取选过的角色
	public List<Record> findByRoleCheck1(String id) {
		List<Record> records = JCacheKit.get(GroupRoleRel.CACHE_NAME, RedisConsts.REDIS_KEY_GROUP_ROLE_RELS_RECORD.concat(id));
		if(records != null && records.size() > 0)
			return records;
		StringBuilder stringBuilder = new StringBuilder("SELECT * FROM group_role_rel gr WHERE gr.group_id = ?");
		records = Db.find(stringBuilder.toString(), id);
		if(records != null && records.size() > 0)
			JCacheKit.put(GroupRoleRel.CACHE_NAME, RedisConsts.REDIS_KEY_GROUP_ROLE_RELS_RECORD.concat(id), records);
		return records;
	}
